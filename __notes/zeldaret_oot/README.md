...
commit: 2c1d2d74bc19c7c5671d866eb7b0e754fa099c28  
...
- `extract.bat`
	- read instructions
	- run it to parse ROM assets
	- it's not strictly necessary, but will improve the experience
...
still, the code gives the insights, that as always the topmost goal is to ship
great experience, not the "cleanest" source. besides, there are cool tricks to learn
```
...
file read: SRAM
file write: sizeof(SaveContext) + 40

slot save: sizeof(SaveContext) + 40
slot load: sizeof(Save)

checksum: sum of `sizeof(Save) / 2` u16 words

@note:
the very first thing to notice is that there's alway a runtime save struct present,
which can be easily written as a buffer to a save file. simple, yet so brilliant!
```
...
## `game.c` -> GameState_Init()
	...
	-> callback GameStateOverlay.init( gameState )
		// from the `gGameStateOverlayTable[]`
...
## `z_play.c` -> Play_Init()
	...
	-> set `GameState.destroy = Play_Destroy`
		...
		-> KaleidoScopeCall_Destroy() @ `z_kaleido_scope.c`
		...
		-> sFileSelectUpdateFuncs[menuMode]()
			...
			-> or FileSelect_SelectModeUpdate()
				-> sSelectModeUpdateFuncs[selectMode]()
					-> or FileSelect_FadeMainToSelect
					-> or FileSelect_MoveSelectedFileToTop
					-> or FileSelect_FadeInFileInfo
					-> or FileSelect_ConfirmFile,
					-> or FileSelect_FadeOutFileInfo
					-> or FileSelect_MoveSelectedFileToSlot
					-> or FileSelect_FadeOut
...
## `z_demo.c` -> Cutscene_UpdateScripted()
	-> if `Save.cutsceneIndex >= 0xFFF0`
		...
		-> callback sScriptedCutsceneHandlers[ CutsceneContext.state ]
			-> CutsceneHandler_StartScript() when CS_STATE_START
			-> CutsceneHandler_RunScript()   when CS_STATE_RUN
			-> CutsceneHandler_StopScript()  when CS_STATE_STOP
			-> CutsceneHandler_RunScript()   when CS_STATE_RUN_UNSTOPPABLE
...
## `z_demo.c` -> Cutscene_UpdateManual()
	-> if `Save.cutsceneIndex < 0xFFF0`
		...
		-> callback sManualCutsceneHandlers[ CutsceneContext.state ]
			-> CutsceneHandler_StartManual() when CS_STATE_START
			-> CutsceneHandler_StopManual()  when CS_STATE_STOP
...
## `z_select.c` -> MapSelect_PrintCutsceneSetting()
	-> Save.cutsceneIndex
		...
		case 0x0000 "Stage: Yoru Goron" ?? CLOCK_TIME(  0, 0 )
		case 0x8000 "Stage: Ohiru Jara" ?? CLOCK_TIME( 12, 0 )
...
## `z_kaleido_scope.c` -> KaleidoScopeCall_Init()
	-> KaleidoManager_GetRamAddr( KaleidoScope_Update )
		// (?) is it a call or not
		-> switch PauseContext.state
			-> case PAUSE_STATE_17
				-> if `something`
					-> SET_NEXT_GAMESTATE( TitleSetup_Init ) @ `z_opening.c`
			-> case ...
	-> KaleidoManager_GetRamAddr( KaleidoScope_Draw )
		// (?) is it a call or not
	-> KaleidoSetup_Init()
